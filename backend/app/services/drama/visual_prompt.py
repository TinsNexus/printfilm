...
4. 贴合故事类型（古装神话/都市悬疑等）与画面风格美学，适合白底全身角色定妆照
...
2. 须写清：空间类型、时代感、前景/中景/背景层次、建筑或地貌、关键陈设、光影、色调、氛围
3. 以环境为主体，不写人物特写；可写无人痕迹（脚印、余烬、法阵光痕）
4. 结合场戏摘录中的 △ 动作与道具，还原可拍摄的空间
5. 禁止「影视级写实」「构图层次分明」等空泛套话"""

PROP_VISUAL_SYSTEM = """你是短剧道具美术，为 Seedream 写道具静物生图描述。
输出 100–220 字简体中文：材质、形制、尺寸感、色彩、磨损做旧、戏剧符号、背景简洁、适合 1:1 特写。不要 JSON。"""
