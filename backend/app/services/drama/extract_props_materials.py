"""从剧本摘要与分集正文抽取道具、素材（对齐 manju extractPropsMaterials）。"""
...
SYSTEM_PROMPT = """你是短剧美术统筹，负责从剧本摘要与分集正文中整理「道具」与「素材」资产清单，供后续 AI 生图使用。
...
素材（materials）：
- 适合作为空镜、气氛板、环境静帧的画面（洪水、刑场远景、山裂、水下幻境等）
- 控制在 6–12 个，强调可拍摄的构图与氛围
- 不要重复罗列与道具清单相同的物件特写

输出要求：
1. 名称简短有辨识度；visualPrompt 用简体中文，每条 90–200 字，可直接作生图提示词
2. visualPrompt 须含：材质/形制、色彩、尺度、磨损或做旧、戏剧符号、建议构图（特写/俯拍等）
3. 必须输出严格 JSON 对象（不要 markdown）：{"props":[{"name":"...","visualPrompt":"..."}],"materials":[{"name":"...","visualPrompt":"..."}]}
"""
...
def _normalize_payload(raw: Any) -> dict[str, list[dict[str, Any]]]:
    ...
    return {"props": read_list("props"), "materials": read_list("materials")}
...
async def extract_props_materials(
    *,
    ...
    return {
        ...
        "materials": _dedupe_by_name(normalized["materials"]),
    }
