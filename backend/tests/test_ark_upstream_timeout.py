...
def test_image_read_timeout_is_at_least_10_minutes():
    assert IMAGE_GEN_READ_SEC >= 600.0
...
def test_reraise_read_timeout_says_waiting_not_unreachable():
    with pytest.raises(RuntimeError, match="等待上游超时") as ei:
        reraise_upstream_timeout(httpx.ReadTimeout(""), kind="生图", read_sec=600)
    text = str(ei.value)
...
def test_reraise_write_timeout_says_send_not_wait():
    with pytest.raises(RuntimeError, match="发送请求超时") as ei:
        reraise_upstream_timeout(httpx.WriteTimeout(""), kind="生图", read_sec=600)
