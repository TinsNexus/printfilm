...
export function DramaSubtitleBoard({
  fragments,
  episodeName = tr('subtitleBoard.episode'),
  subtitleMode,
}: Props) {
  const { t: tx } = useI18n()
      ...
      <div className="drama-subtitle-board__header">
        ...
        >
          <span className="drama-subtitle-board__title-wrap">
            <h4>{tx('subtitleBoard.subtitleBoard')}</h4>
            <p>
              {tx('subtitleBoard.cueCount', { mode: modelOutput ? tx('subtitleBoard.modelRendered') : tx('subtitleBoard.postProduction'), n: cues.length })}
            </p>
          </span>
          {collapsed ? (
        ...
        <button
          ...
          title={tx('subtitleBoard.exportSrtImportedInto')}
          onClick={(event) => {
            ...
            triggerBlobDownload(blob, `${sanitizeMediaBasename(episodeName)}${tx('subtitleBoard.srtSuffix')}`)
          }}
        >
          {tx('subtitleBoard.exportSrt')}
        </button>
      </div>
      {!collapsed ? (
        cues.length === 0 ? (
          <div className="drama-subtitle-board__empty">{tx('subtitleBoard.dialogueNarrationSubtitlesPreview')}</div>
        ) : (
            ...
            {cues.map((cue, index) => (
              ...
              >
                <div className="drama-subtitle-board__meta">
                  ...
                  <span>{tx('subtitleBoard.clipN', { n: String(cue.fragmentIndex + 1).padStart(2, '0') })}</span>
